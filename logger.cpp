					...
					const std::string& message)
{
	...
	Logger::lock();
	std::string vl="";
    if (verbosityLevel==LoggerNS::DBG_DEBUG) vl="DEBUG";
    else if (verbosityLevel==LoggerNS::DBG_WARN) vl="WARN";
    else if (verbosityLevel==LoggerNS::DBG_INFO) vl="INFO";
    else vl="ERROR";
	if ((configuration_&file_on) && (verbosityLevel <= fileVerbosityLevel_))
			out_ << vl <<" [" << file << ":" << line <<  ":" << func<<"] @ " <<
				(currentTime.tv_sec - initialTime_.tv_sec) <<
				":" << message << std::endl;

	if ((configuration_&screen_on) && (verbosityLevel <= screenVerbosityLevel_))
			std::cerr << vl<<" [" << file << ":" << line << ":" << func<<"] @ " <<
				(currentTime.tv_sec - initialTime_.tv_sec) <<
				":" << message << std::endl;
	Logger::unlock();
}
